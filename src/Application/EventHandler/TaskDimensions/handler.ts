...
import { ServiceTaskDimensions } from "../../../Domain/Services/Task/ServiceTaskDimensions";
...
containerBuilder.addDefinitions([
  {
    ...
    value: (container: ContainerInterface) => {
      ...
      const config: TracingConfig = {
        serviceName: settings.CONSUMER_GROUP_ID_TOPIC_DIMENSIONS,
        sampler: {
          type: "const",
          param: 1,
        },
...
const topic: string = container.get(
  IoC.Settings
).CONSUMER_SUBSCRIBE_TOPIC_DIMENSIONS;

const consumer = kafka.consumer({
  groupId: container.get(IoC.Settings).CONSUMER_GROUP_ID_TOPIC_DIMENSIONS,
    ...
    eachBatch: (payload: EachBatchPayload): Promise<void> => {
      ...
      const actions = PromiseB.map(
        ...
        (message: KafkaMessage) => {
          return PromiseB.try(() => {
            return new ServiceTaskDimensions({
              container: container,
            }).execute({
